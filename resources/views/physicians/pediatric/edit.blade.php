...
<div class="row mt-5">
        <div class="col-md-8 mx-auto">
            ...
            <hr>
        <p><strong> Name : </strong>{{$pediatricBasicInfo->patients->patientName}}</p>
        <p><strong> Date of Birth : </strong>{{ \Carbon\Carbon::parse($pediatricBasicInfo->patients->dob)->format('d/m/Y')}}</p>
        <p><strong>Age:</strong><span> {{\Carbon\Carbon::parse($pediatricBasicInfo->patients->dob)->diff(\Carbon\Carbon::now())->format('%y years, %m months and %d days')}}</span></p>
        <p><strong>Sex:</strong> {{$pediatricBasicInfo->patients->patientSex}}</p>

            <hr>
            <h4 class="text-center">Family History</h4>
            <!--the basic pediatric info form-->

            {!! Form::model($pediatricBasicInfo,['route' => ['edit_basic_info.update',$pediatricBasicInfo->id],'method'=>'PUT','data-parsley-validate'=>'']) !!}
