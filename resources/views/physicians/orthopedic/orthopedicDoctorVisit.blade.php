...
<div class="row mt-5">
    ...
    <p><strong>Sex:</strong> {{$patient->patientSex}}</p>
        ...
        {!! Form::open(['route' => ['orthopedicVisit.store',$patient->id],'data-parsley-validate'=>'']) !!}

        <div class="form-group">
            {{Form::label('co', 'C/O')}}
            {{ Form::text('co',null, ['class' => 'form-control']) }}
        </div>

        <div class="form-group">
            {{Form::label('clinical_finding', 'Clinical Finding')}}
            {{ Form::text('clinical_finding',null, ['class' => 'form-control']) }}          
        </div>
